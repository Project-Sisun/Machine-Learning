...
# Reshape -> (?, 8 * 8 * FN) - Flatten them for FC
L2_flat = tf.reshape(L2, [-1, 8 * 8 * FN])

# ---------------------------------------------------------------------------------------------------
# L3 FC!!! 8x8xFN inputs -> 1 outputs
W3 = tf.get_variable("W3", shape=[8 * 8 * FN, nb_classes], initializer=tf.contrib.layers.xavier_initializer())
...
writer = tf.summary.FileWriter('./logs/2LayerPool')

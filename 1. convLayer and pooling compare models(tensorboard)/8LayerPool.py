...
# max_pool 2*2, strides 2*2 - STOP POOLING AT 2*2*_ IMAGE SHAPE
# No dropout
# No ensemble
...
# ---------------------------------------------------------------------------------------------------
# L8 ImgIn shape = (?, 2, 2, FN)
W8 = tf.Variable(tf.random_normal([FS, FS, FN, FN], stddev=0.01))

# Conv -> (?, 2, 2, FN)
L8 = tf.nn.conv2d(L7, W8, strides=[1, 1, 1, 1], padding='SAME')
L8 = tf.nn.relu(L8)

# Reshape -> (?, 2 * 2 * FN) - Flatten them for FC
L8_flat = tf.reshape(L8, [-1, 2 * 2 * FN])

# ---------------------------------------------------------------------------------------------------
# L9 FC!!! 2x2xFN inputs -> 1 outputs
W9 = tf.get_variable("W9", shape=[2 * 2 * FN, nb_classes], initializer=tf.contrib.layers.xavier_initializer())
b9 = tf.Variable(tf.random_normal([nb_classes]))
logits = tf.matmul(L8_flat, W9) + b9
...
writer = tf.summary.FileWriter('./logs/8LayerPool')

...
L6_flat = tf.reshape(L6, [-1, 2 * 2 * FN])

# ---------------------------------------------------------------------------------------------------
# L7 FC!!! 2x2xFN inputs -> 1 outputs
W7 = tf.get_variable("W7", shape=[2 * 2 * FN, nb_classes], initializer=tf.contrib.layers.xavier_initializer())
b7 = tf.Variable(tf.random_normal([nb_classes]))
logits = tf.matmul(L6_flat, W7) + b7
...
writer = tf.summary.FileWriter('./logs/6LayerPool')

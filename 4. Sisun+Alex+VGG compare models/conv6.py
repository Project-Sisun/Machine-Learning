...
start = time.time()
logdir = './logs/conv6/' + str(time.time())

np.random.seed(int(time.time()))
tf.set_random_seed(777)  # reproducibility

# hyper parameters
inputDataType = "Conv6_YCbCrR"
learning_rate = 0.0009
# training_epochs = 200 (cost 수렴 epoch)
training_epochs = 200
batch_size = 4000
image_depth = 3
...
save_path = saver.save(sess, os.getcwd() + "/" + dirName + "/conv6_" + str(
...
f = open("./(구)history.txt", 'a')
f.write('-- conv6 %d depth-----------------------\n' % image_depth)

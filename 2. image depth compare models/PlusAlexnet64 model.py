...
start = time.time()
logdir = './logs/64alexnet/' + str(time.time())
...
# img 64x64x1 (black/white)
X_img = tf.reshape(X, [-1, 64, 64, image_depth])

# ---------------------------------------------------------------------------------------------------
# L1 ImgIn shape = (?, 64, 64, image_depth)
W1 = tf.Variable(tf.random_normal([11, 11, image_depth, 96], stddev=0.01))
...
# ---------------------------------------------------------------------------------------------------
# L2 ImgIn shape = (?, 32, 32, 96)
W2 = tf.Variable(tf.random_normal([5, 5, 96, 256], stddev=0.01))
...
# Reshape -> (?, 8 * 8 * 256) - Flatten them for FC
L5_flat = tf.reshape(L5, [-1, 8 * 8 * 256])

# ---------------------------------------------------------------------------------------------------
# L6 FC 8x8x256 inputs ->  4096 outputs
W6 = tf.get_variable("W6", shape=[256 * 8 * 8, 4096], initializer = tf.contrib.layers.xavier_initializer())
...
save_path = saver.save(sess, os.getcwd()+"/64alexnet.ckpt")

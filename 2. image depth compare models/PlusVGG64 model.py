...
start = time.time()
logdir = './logs/64VGG/' + str(time.time())
...
L5_flat = tf.reshape(L5, [-1, 2 * 2 * 512])

# ---------------------------------------------------------------------------------------------------
# L6 FC 2x2x512 inputs ->  4096 outputs
W6 = tf.get_variable("W6", shape=[512 * 2 * 2, 4096], initializer = tf.contrib.layers.xavier_initializer())
...
save_path = saver.save(sess, os.getcwd()+"/64VGG.ckpt")

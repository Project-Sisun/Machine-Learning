...
start = time.time()
logdir = './logs/128VGG/' + str(time.time())
...
L5_flat = tf.reshape(L5, [-1, 4 * 4 * 512])

# ---------------------------------------------------------------------------------------------------
# L6 FC 4x4x512 inputs ->  4096 outputs
W6 = tf.get_variable("W6", shape=[512 * 4 * 4, 4096], initializer = tf.contrib.layers.xavier_initializer())
...
save_path = saver.save(sess, os.getcwd()+"/128VGG.ckpt")

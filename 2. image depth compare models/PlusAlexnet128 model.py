...
start = time.time()
logdir = './logs/128alexnet/' + str(time.time())
...
# ---------------------------------------------------------------------------------------------------
# X: input 128*128*image_depth
# Y: output '1' or '0'
X = tf.placeholder(tf.float32, [None, 128*128*image_depth])
...
# img 128x128x1 (black/white)
X_img = tf.reshape(X, [-1, 128, 128, image_depth])

# ---------------------------------------------------------------------------------------------------
# L1 ImgIn shape = (?, 128, 128, image_depth)
...
L5_flat = tf.reshape(L5, [-1, 16 * 16 * 256])

# ---------------------------------------------------------------------------------------------------
# L6 FC 16x16x256 inputs ->  4096 outputs
W6 = tf.get_variable("W6", shape=[256 * 16 * 16, 4096], initializer = tf.contrib.layers.xavier_initializer())
...
save_path = saver.save(sess, os.getcwd()+"/128alexnet.ckpt")

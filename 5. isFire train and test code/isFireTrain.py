...
start = time.time()
logdir = './logs/isFire/' + str(time.time())
...
for epoch in range(training_epochs):
    ...
    while i <= train_N:
        ...
        if Training_Cost <= (0.03*saveCkpt):
            saver.save(sess, os.getcwd() + "/" + dirName + "/ISFire_epoch" + str(epoch) + "_batch" + str(i) + "_cost" + str(Training_Cost) + ".ckpt")

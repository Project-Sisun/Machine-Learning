...
start = time.time()
logdir = './logs/new3/' + str(time.time())
...
# lrn3
# lrn(2, 2e-05, 0.75, name='norm1')
L3 = tf.nn.local_response_normalization(L3, depth_radius=radius, alpha=alpha, beta=beta, bias=bias)
...
# lrn4
# lrn(2, 2e-05, 0.75, name='norm1')
L4 = tf.nn.local_response_normalization(L4, depth_radius=radius, alpha=alpha, beta=beta, bias=bias)
...
# lrn5
# lrn(2, 2e-05, 0.75, name='norm1')
L5 = tf.nn.local_response_normalization(L5, depth_radius=radius, alpha=alpha, beta=beta, bias=bias)
...
# ---------------------------------------------------------------------------------------------------
# L6 FC!!! 4x4xFN inputs -> 1 outputs
W6 = tf.get_variable("W6", shape=[4 * 4 * FN, nb_classes], initializer=tf.contrib.layers.xavier_initializer())
b6 = tf.Variable(tf.random_normal([nb_classes]))
logits = tf.matmul(L5_flat, W6) + b6
...
save_path = saver.save(sess, os.getcwd()+"/0new3.ckpt")
print("\nModel saved in file: %s" % save_path)

sess.close()

f = open("./history.txt", 'a')
f.write('-- new3 %d depth-----------------------\n' % image_depth)

...
start = time.time()
logdir = './logs/32alexnet/' + str(time.time())
...
L5_flat = tf.reshape(L5, [-1, 4 * 4 * 256])

# ---------------------------------------------------------------------------------------------------
# L6 FC 4x4x256 inputs ->  4096 outputs
W6 = tf.get_variable("W6", shape=[256 * 4 * 4, 4096], initializer = tf.contrib.layers.xavier_initializer())
...
save_path = saver.save(sess, os.getcwd()+"/32alexnet.ckpt")
print("\nModel saved in file: %s" % save_path)

sess.close()

f = open("./history.txt", 'a')
f.write('-- 32alexnet %d depth-----------------------\n' % image_depth)

...
start = time.time()
logdir = './logs/new4/' + str(time.time())
...
L4_flat = tf.reshape(L4, [-1, 2 * 2 * FN])

# ---------------------------------------------------------------------------------------------------
# L5 FC!!! 8x8xFN inputs -> 1 outputs
W5 = tf.get_variable("W5", shape=[2 * 2 * FN, nb_classes], initializer=tf.contrib.layers.xavier_initializer())
...
save_path = saver.save(sess, os.getcwd()+"/0new4.ckpt")
print("\nModel saved in file: %s" % save_path)

sess.close()

f = open("./history.txt", 'a')
f.write('-- new4 %d depth-----------------------\n' % image_depth)

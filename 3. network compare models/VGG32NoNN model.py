...
start = time.time()
logdir = './logs/32VGGNoNN/' + str(time.time())
...
W6 = tf.get_variable("W6", shape=[512 * 1 * 1, nb_classes], initializer=tf.contrib.layers.xavier_initializer())
...
save_path = saver.save(sess, os.getcwd()+"/032VGGNoNN.ckpt")
print("\nModel saved in file: %s" % save_path)

sess.close()

f = open("./history.txt", 'a')
f.write('-- 32VGG(NoNN) %d depth-----------------------\n' % image_depth)
